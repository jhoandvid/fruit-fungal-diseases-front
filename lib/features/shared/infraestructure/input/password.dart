...
class ConfirmPassword extends FormzInput<String, PasswordError> {
  ...
  // Constructor that takes the original password as a parameter
  const ConfirmPassword.pure({this.password}) : super.pure('');
  const ConfirmPassword.dirty(this.password, String value) : super.dirty(value);

   String? get errorMessage {
    if ( isValid || isPure ) return null;
    if ( displayError == PasswordError.mismatch ) return 'Las contraseñas deben ser iguales';
  ...
  @override
  PasswordError? validator(String value) {
    if (value != password!.value) return PasswordError.mismatch;
    return null;
  }
}